...
python .ua\build_graph.py
echo [3/4] layers + tour
python .ua\enrich.py
echo [4/4] stamp hash
for /f %%h in (`"git rev-parse HEAD"`) do set HASH=%%h
python -c "import json,datetime,pathlib; p=pathlib.Path(r\".ua\knowledge-graph.json\"); g=json.loads(p.read_text(encoding=\"utf-8\")); g[\"project\"][\"gitCommitHash\"]=\"%HASH%\"; g[\"project\"][\"analyzedAt\"]=datetime.datetime.now(datetime.timezone.utc).isoformat(); p.write_text(json.dumps(g,ensure_ascii=False,indent=2),encoding=\"utf-8\"); print(\"hash=\"+\"%HASH%\")"
git add -A
